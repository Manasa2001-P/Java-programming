...
public class Mergesort{
    ...
    public static void mergeSort(int a[], int lb, int mid, int ub){
        int i;                    // indicates the starting position of the left sublist
        ...
        while(j<=ub){        // case 3, dumping remaining elements in sublist 2
            temp[k]=a[j];
            k++; j++;
        }
        for(i=lb;i<=ub;i++){ a[i]=temp[i]; }            // post all the elements from temp to back orginial array 'a'
    }
